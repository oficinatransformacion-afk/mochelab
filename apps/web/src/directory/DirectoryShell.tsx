...
export const demoProfile: DemoProfile = storedProfile === "SYSTEM" ? "SYSTEM" : storedProfile === "ADMIN" ? "ADMIN" : storedProfile === "FACILITADOR" ? "FACILITADOR" : storedProfile === "COLABORADOR" ? "COLABORADOR" : import.meta.env.VITE_DEMO_PROFILE === "SYSTEM" ? "SYSTEM" : import.meta.env.VITE_DEMO_PROFILE === "ADMIN" ? "ADMIN" : import.meta.env.VITE_DEMO_PROFILE === "FACILITADOR" ? "FACILITADOR" : "COLABORADOR";
const sessionToken=sessionStorage.getItem(SESSION_TOKEN_KEY);
export const demoHeaders = { "x-mochelab-demo-profile":demoProfile, "x-mochelab-demo-user-email": sessionStorage.getItem(DEMO_EMAIL_KEY) ?? import.meta.env.VITE_DEMO_USER_EMAIL ?? (demoProfile==="SYSTEM"?"system.prueba@example.invalid":demoProfile==="ADMIN"?"admin.prueba@example.invalid":demoProfile==="FACILITADOR"?"facilitador.prueba@example.invalid":"usuario.prueba@example.invalid"),...(sessionToken?{Authorization:`Bearer ${sessionToken}`}:{}) };
