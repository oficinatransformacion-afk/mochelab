    ...
    {data && <div className="mt-8 space-y-5">
      ...
      {summary.assignments.length ? summary.assignments.map(assignment => {
        return <section key={assignment.id} className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          ...
          <div className="p-6"><div className="rounded-xl border border-slate-100 bg-slate-50 p-5"><div className="flex items-center gap-2"><Award size={18} className="text-[#d71920]" /><h3>Contexto de la asignación</h3></div><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-3"><div><dt className="text-slate-500">Rol</dt><dd className="font-bold">{assignment.role.name}</dd></div><div><dt className="text-slate-500">Equipo</dt><dd className="font-bold">{assignment.team.sourceId} · {assignment.team.program.name}</dd></div><div><dt className="text-slate-500">Onboarding</dt><dd className="font-bold">{assignment.onboardingStatus.name}</dd></div></dl><p className="mt-5 text-xs text-slate-500">La madurez y la ruta de aprendizaje del rol se muestran una sola vez en la sección superior.</p></div></div>
        </section>;
