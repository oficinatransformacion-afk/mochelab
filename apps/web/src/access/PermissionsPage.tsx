...
export function PermissionsPage() {
  ...
  async function change(module:Module,key:typeof labels[number][0],checked:boolean){
    if(!profile)return;
    if(!checked){setConfirmError("");setPending({module,key});return}
    await persist(module,key,checked);
  }
  async function persist(module:Module,key:typeof labels[number][0],checked:boolean){
    if(!profile)return;
    setBusy(true);
    ...
    if(!response.ok){const body=await response.json().catch(()=>({}));const text=body.message??"No se pudo guardar el permiso";setMessage(text);setConfirmError(text)}
    else{setMessage(`Permisos de ${profile.name} actualizados.`);setPending(null);await load()}
    setBusy(false);
  }
  ...
  </div>{pending&&profile&&<ConfirmationDialog title="Retirar permiso" description="Revisa el alcance antes de modificar el perfil." icon={<ShieldMinus size={20}/>} confirmLabel="Retirar permiso" danger busy={busy} error={confirmError} onClose={()=>setPending(null)} onConfirm={()=>void persist(pending.module,pending.key,false)}><div className="rounded-xl bg-slate-50 p-4"><strong>{menuNames(pending.module.code).join(" · ")||pending.module.name}</strong><span className="mt-1 block text-sm text-slate-500">Perfil {profile.name} · Permiso: {labels.find(([key])=>key===pending.key)?.[1]}</span></div>{pending.key==="canView"&&<p className="mt-4 text-sm text-rose-800">Al retirar “Ver” también se desactivarán Crear, Editar y Eliminar para este módulo.</p>}</ConfirmationDialog>}</main>;
}
