...
export function CalibrationPage() {
  ...
  const [data, setData] = useState<Detail | null>(null), [score, setScore] = useState(0), [comments, setComments] = useState(""), [message, setMessage] = useState(""), [confirming, setConfirming] = useState(false), [saving, setSaving] = useState(false), [confirmError, setConfirmError] = useState("");
  ...
  useEffect(() => {
    ...
    }).then(item => { setData(item); setScore(item.calibratedScore ?? item.originalScore); setComments(item.calibrationComments ?? ""); }).catch((error: Error) => setMessage(error.message));
  }, [id]);
  ...
  async function save() {
    ...
    try {
      ...
      if (response.ok) { setData({ ...data!, calibratedScore: score, calibratedAt: new Date().toISOString(), calibrationComments: comments }); setMessage(`Calibración guardada. Nivel final: ${body.level}.`); setConfirming(false); }
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-950"><header className="border-b bg-white"><div className="mx-auto flex max-w-7xl justify-between px-5 py-4"><a href="/madurez/calibraciones" className="font-bold">← Calibraciones</a><b>Administrador</b></div></header><div className="mx-auto max-w-7xl px-5 py-8"><h1 className="text-3xl font-bold">Revisión y calibración</h1><p className="mt-2 text-slate-600">{data.person} · {data.role} · {data.team} · {data.period}</p><div className="mt-8 grid gap-7 2xl:grid-cols-[minmax(0,1fr)_22rem]"><section className="space-y-5"><DimensionScoreChart details={data.resultDetails ?? []} responses={data.responses}/><details className="rounded-2xl bg-white p-5 shadow-sm"><summary className="cursor-pointer font-bold text-slate-700">Ver respuestas originales ({data.responses.length})</summary><div className="mt-4 max-h-[34rem] overflow-y-auto pr-2">{data.responses.map(item => <div key={item.behaviorId} className="border-t py-4"><p className="text-sm font-semibold text-emerald-700">{item.dimensionName}</p><p className="mt-1">{item.statement}</p><p className="mt-2 font-bold">Puntaje: {item.score}</p>{item.comments && <p className="text-sm text-slate-500">{item.comments}</p>}</div>)}</div></details>
    ...
    <article className="rounded-2xl bg-white p-5 shadow-sm"><label className="font-bold">Comentario de calibración<textarea disabled={readonly} rows={5} value={comments} onChange={event => setComments(event.target.value)} className="mt-3 w-full rounded-xl border p-3 font-normal disabled:bg-slate-50" placeholder="Obligatorio si cambias el puntaje"/></label></article></section><aside><div className="rounded-2xl bg-slate-950 p-6 text-white"><p className="text-sm text-emerald-400">Puntaje enviado</p><p className="text-4xl font-bold">{data.originalScore.toFixed(2)}</p><label className="mt-6 block text-sm font-semibold text-emerald-400">Puntaje calibrado<input aria-label="Puntaje calibrado" disabled={readonly} type="number" min="0" max="2" step=".01" value={score} onChange={event => setScore(Number(event.target.value))} className="mt-2 w-full rounded-xl border border-slate-600 bg-slate-900 p-3 text-3xl font-bold text-white caret-white [-webkit-text-fill-color:white] disabled:cursor-not-allowed disabled:border-slate-700 disabled:bg-slate-800 disabled:opacity-100"/></label><div className="mt-3 flex items-center justify-between text-sm"><span className="text-slate-400">Nivel resultante</span><strong className="text-white">{resultingLevel(score)}</strong></div>{readonly && <p className="mt-4 rounded-lg bg-emerald-950 p-3 text-sm font-semibold text-emerald-300">Calibración final guardada</p>}<button disabled={!valid} onClick={() => { setConfirmError(""); setConfirming(true); }} className="mt-6 w-full rounded-xl bg-emerald-500 p-3 font-bold text-slate-950 disabled:bg-slate-700 disabled:text-slate-400">Guardar calibración</button></div>{message && <p className="mt-4 rounded-xl bg-emerald-50 p-4 font-semibold text-emerald-900">{message}</p>}</aside></div></div>{confirming && <ConfirmationDialog title="Confirmar calibración final" description="Este resultado quedará cerrado para el período seleccionado." icon={<Scale size={20}/>} confirmLabel="Guardar resultado final" busy={saving} error={confirmError} onClose={() => setConfirming(false)} onConfirm={() => void save()}><div className="rounded-xl bg-slate-50 p-4"><strong>{data.person}</strong><span className="mt-1 block text-sm text-slate-500">{data.role} · {data.team} · {data.period}</span></div><dl className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl border p-3"><dt className="text-xs font-bold uppercase text-slate-500">Puntaje enviado</dt><dd className="mt-1 text-2xl font-bold">{data.originalScore.toFixed(2)}</dd></div><div className="rounded-xl border p-3"><dt className="text-xs font-bold uppercase text-slate-500">Puntaje final</dt><dd className="mt-1 text-2xl font-bold">{score.toFixed(2)}</dd></div></dl><p className="mt-4 text-sm text-slate-700">Después de guardar, la calibración no podrá modificarse.</p></ConfirmationDialog>}</main>;
}
