...
import { BarChart3, CheckCircle2, ChevronRight, Scale, Search, SlidersHorizontal } from "lucide-react";
...
type Assessment = { id: string; person: string; role: string; team: string; period: string; score: number; submittedAt: string; status: ReviewStatus };
...
type CalibrationDetail = { id: string; person: string; role: string; team: string; period: string; originalScore: number; calibratedScore: number | null; calibratedAt: string | null; calibrationComments: string | null; level: string; responses: ResponseRow[]; resultDetails: ResultDetail[] };
...
export function CalibrationQueuePage() {
  ...
  }, [assessments, period, search, status, team]);
  ...
  const periodRows = assessments.filter((row) => period === "TODOS" || row.period === period);
  const calibrated = periodRows.filter((row) => row.status === "CALIBRADA").length;
  const pending = periodRows.filter((row) => row.status === "PENDIENTE").length;
  ...
  const valid = Boolean(detail) && !readonly && Number.isFinite(score) && score >= 0 && score <= 2 && (!changed || Boolean(comments.trim()));
    ...
    <div className="mx-auto max-w-[96rem] px-5 py-8 lg:px-8">
      ...
      <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid gap-5 lg:grid-cols-[minmax(15rem,22rem)_1fr] lg:items-center">
          <label className="text-sm font-bold text-slate-700">Período de evaluación<select value={period} onChange={(event) => { setPeriod(event.target.value); setTeam("TODOS"); }} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 font-normal"><option value="TODOS">Todos los períodos</option>{[...new Set(assessments.map((row) => row.period))].map((value) => <option key={value}>{value}</option>)}</select></label>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(22rem,.82fr)_minmax(34rem,1.18fr)] xl:items-start">
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 p-4">
            <div className="flex flex-wrap gap-2" aria-label="Filtrar por estado">{(["PENDIENTE", "CALIBRADA", "TODAS"] as const).map((option) => <button type="button" key={option} onClick={() => setStatus(option)} className={`rounded-full px-3 py-2 text-sm font-semibold ${status === option ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-600"}`}>{option === "PENDIENTE" ? "Pendientes" : option === "CALIBRADA" ? "Calibradas" : "Todas"}</button>)}</div>
            <div className="mt-3 flex gap-2"><label className="relative min-w-0 flex-1"><Search size={18} className="pointer-events-none absolute left-3 top-3 text-slate-400"/><span className="sr-only">Buscar</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Persona, rol o equipo" className="w-full rounded-xl border border-slate-300 py-2.5 pl-10 pr-3 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600"/></label><button type="button" aria-pressed={showTeamFilter} onClick={() => setShowTeamFilter((value) => !value)} className={`rounded-xl border px-3 ${showTeamFilter ? "border-emerald-600 bg-emerald-50 text-emerald-800" : "border-slate-300 text-slate-600"}`} aria-label="Mostrar filtro de equipo"><SlidersHorizontal size={19}/></button></div>
            {showTeamFilter && <select aria-label="Filtrar por equipo" value={team} onChange={(event) => setTeam(event.target.value)} className="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="TODOS">Todos los equipos</option>{[...new Set(assessments.map((row) => row.team))].map((value) => <option key={value}>{value}</option>)}</select>}
          </div>
        ...
        <section className="xl:sticky xl:top-5">
          ...
          {!loadingDetail && detail && <div className="space-y-5">
            <article className="rounded-2xl bg-slate-950 p-5 text-white shadow-sm"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-sm font-semibold text-emerald-400">Persona y rol</p><h2 className="mt-1 text-2xl font-bold">{detail.person}</h2><p className="mt-1 text-slate-300">{detail.role} · {detail.team} · {detail.period}</p></div><span className={`w-fit rounded-full px-3 py-1 text-sm font-bold ${readonly ? "bg-emerald-400 text-emerald-950" : "bg-amber-300 text-amber-950"}`}>{readonly ? "Calibrada" : "Pendiente"}</span></div></article>
            <DimensionSummary detail={detail}/>
            <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm text-slate-500">Puntaje enviado</p><strong className="mt-1 block text-3xl">{detail.originalScore.toFixed(2)}</strong><span className="text-sm text-slate-500">{levelFor(detail.originalScore)}</span></div><label className="rounded-xl bg-emerald-50 p-4 text-sm font-bold text-emerald-900">Puntaje calibrado<input aria-label="Puntaje calibrado" disabled={readonly} type="number" min="0" max="2" step=".01" value={scoreInput} onChange={(event) => { setScoreInput(event.target.value); setScore(event.target.value.trim() ? Number(event.target.value) : Number.NaN); }} onBlur={() => { if (Number.isFinite(score)) setScoreInput(score.toFixed(2)); }} className="mt-2 w-full rounded-xl border border-emerald-300 bg-white p-3 text-2xl font-bold text-slate-950 disabled:bg-slate-100"/><span className="mt-2 block text-right">Nivel: {finalLevel}</span></label></div>
              <label className="mt-5 block text-sm font-bold text-slate-700">Justificación del ajuste {changed && <span className="text-rose-700">(obligatoria)</span>}<textarea disabled={readonly} rows={3} value={comments} onChange={(event) => setComments(event.target.value)} placeholder="Describe la evidencia que sustenta el ajuste" className={`mt-2 w-full rounded-xl border p-3 font-normal ${changed && !comments.trim() ? "border-rose-400" : "border-slate-300"}`}/></label>
