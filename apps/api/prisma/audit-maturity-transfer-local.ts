...
async function main() {
  ...
  const assessmentDuplicates = groupDuplicates(assessmentRows);
  const maturityDuplicates = groupDuplicates(maturityRows);
  const report = {
    sourceDatabase,
    ...
    validation:{
      ...
      selfAssessmentsWithoutMaturity:assessments.filter(row => !maturityByAssessment.has(row.id)).map(row => ({ companyCode:row.personRole.person.company.code, personDni:row.personRole.person.dni, roleCode:row.personRole.role.sourceId, periodCode:row.period.code })),
    },
