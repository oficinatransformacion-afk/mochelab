...
export class DirectoryRepository {
  ...
  async listAssignments(filters:{search?:string;teamIds?:string[];roleIds?:string[];statusIds?:string[];onboardingStatusIds?:string[]},teamIds:string[]|null=null){
    const term=filters.search?.trim();
    return this.prisma.personRole.findMany({where:{teamId:filters.teamIds?.length?{in:filters.teamIds}:undefined,roleId:filters.roleIds?.length?{in:filters.roleIds}:undefined,statusId:filters.statusIds?.length?{in:filters.statusIds}:undefined,onboardingStatusId:filters.onboardingStatusIds?.length?{in:filters.onboardingStatusIds}:undefined,AND:teamIds===null?undefined:[{teamId:{in:teamIds}}],OR:term?[{person:{names:{contains:term,mode:"insensitive"}}},{person:{dni:{contains:term,mode:"insensitive"}}},{role:{name:{contains:term,mode:"insensitive"}}},{team:{sourceId:{contains:term,mode:"insensitive"}}}]:undefined},include:{person:{include:{company:true}},role:true,team:{include:{program:true}},status:true,onboardingStatus:true,_count:{select:{courses:true}}},orderBy:[{person:{names:"asc"}},{role:{name:"asc"}}],take:500});
  }
