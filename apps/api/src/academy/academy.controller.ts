...
export class AcademyController {
  ...
  @AdminOnly() @RequirePermission("CURSOS","create") @Post("courses") async createCourse(@Body() body:Record<string,unknown>,@Headers("x-mochelab-demo-user-email") email?:string){return this.repository.saveCourse(null,body,await this.access.getUserId("ADMINISTRADOR",email))}
  @AdminOnly() @RequirePermission("CURSOS","edit") @Patch("courses/:id") async updateCourse(@Param("id") id:string,@Body() body:Record<string,unknown>,@Headers("x-mochelab-demo-user-email") email?:string){return this.repository.saveCourse(id,body,await this.access.getUserId("ADMINISTRADOR",email))}

  @AdminOnly()
  @RequirePermission("CURSOS","create")
  @Post("roles/:roleId/courses/:courseId")
  async assignCourse(@Param("roleId") roleId: string, @Param("courseId") courseId: string,@Headers("x-mochelab-demo-user-email") email?:string) { return this.repository.assignCourse(roleId, courseId,await this.access.getUserId("ADMINISTRADOR",email)); }
  @AdminOnly() @RequirePermission("CURSOS","edit") @Patch("roles/:roleId/courses/:courseId/deactivate") async unassignCourse(@Param("roleId") roleId:string,@Param("courseId") courseId:string,@Headers("x-mochelab-demo-user-email") email?:string){return this.repository.unassignCourse(roleId,courseId,await this.access.getUserId("ADMINISTRADOR",email))}
  ...
  async updateProgress(@Param("personRoleId") personRoleId: string, @Param("courseId") courseId: string, @Body() body: { statusCode?: string; score?: number | null; startDate?: string | null; endDate?: string | null },@Headers("x-mochelab-demo-user-email") email?:string) {
    ...
    return this.repository.updateProgress(personRoleId, courseId, body.statusCode, body.score, body.startDate, body.endDate,await this.access.getUserId("ADMINISTRADOR",email));
  }
}
