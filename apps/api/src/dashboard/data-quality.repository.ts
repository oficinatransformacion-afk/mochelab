...
export class DataQualityRepository{
 ...
 async list(){
  ...
  const issues:Issue[]=[];
  for(const x of people){if(!x.email)issues.push({type:"PERSONA_SIN_CORREO",severity:"ALTA",entity:"PERSONA",recordId:x.id,label:x.names,detail:`${x.dni} · ${x.company.name}: no tiene correo`,href:`/personas/${x.id}`});if(!x.organizationalUnitId)issues.push({type:"PERSONA_SIN_UNIDAD",severity:"MEDIA",entity:"PERSONA",recordId:x.id,label:x.names,detail:`${x.dni} · ${x.company.name}: no tiene unidad organizacional`,href:`/personas/${x.id}`});if(!x.managementId)issues.push({type:"PERSONA_SIN_GERENCIA",severity:"MEDIA",entity:"PERSONA",recordId:x.id,label:x.names,detail:`${x.dni} · ${x.company.name}: no tiene gerencia asignada`,href:`/personas/${x.id}`});if(!x.divisionId)issues.push({type:"PERSONA_SIN_DIVISION",severity:"MEDIA",entity:"PERSONA",recordId:x.id,label:x.names,detail:`${x.dni} · ${x.company.name}: no tiene división asignada`,href:`/personas/${x.id}`});if(!x.businessPartnerValueId)issues.push({type:"PERSONA_SIN_BUSINESS_PARTNER",severity:"MEDIA",entity:"PERSONA",recordId:x.id,label:x.names,detail:`${x.dni} · ${x.company.name}: no tiene Business Partner asignado`,href:`/personas/${x.id}`})}
  ...
  for(const x of teams){if(!x._count.assignments)issues.push({type:"EQUIPO_SIN_MIEMBROS",severity:"MEDIA",entity:"EQUIPO",recordId:x.id,label:x.sourceId,detail:"Equipo activo sin asignaciones activas",href:"/equipos"});if(!x.focusAreaId)issues.push({type:"EQUIPO_SIN_AREA_ENFOQUE",severity:"MEDIA",entity:"EQUIPO",recordId:x.id,label:`${x.sourceId} · ${x.name}`,detail:"No tiene Área de enfoque asignada",href:"/configuracion/maestros"})}
