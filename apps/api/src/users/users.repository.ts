...
export class UsersRepository{
 ...
 async options(){
  const [values,teams,people]=await Promise.all([
   this.prisma.catalogValue.findMany({where:{active:true,OR:[{catalog:{code:"ESTADO_USUARIO"}},{catalog:{code:"PERFIL_USUARIO"},code:{in:["COLABORADOR","FACILITADOR","ADMIN","SYSTEM"]}}]},orderBy:[{catalog:{code:"asc"}},{sortOrder:"asc"}],select:{id:true,code:true,name:true,catalog:{select:{code:true}}}}),
 ...
 async refs(profileId:string,statusId:string,teamIds:string[],personId:string|null,currentUserId?:string){
  const [profile,status,teams,person]=await Promise.all([
   this.prisma.catalogValue.findFirst({where:{id:profileId,code:{in:["COLABORADOR","FACILITADOR","ADMIN","SYSTEM"]},catalog:{code:"PERFIL_USUARIO"}}}),
  ...
  if(profile.code==="COLABORADOR"&&!personId)throw new BadRequestException("El perfil Colaborador debe vincularse a una persona");
  if(profile.code==="FACILITADOR"&&!teamIds.length)throw new BadRequestException("El perfil Facilitador debe tener al menos un equipo permitido");
 ...
 async create(i:Record<string,unknown>,administratorId:string){
  ...
  const refs=await this.refs(profileId,statusId,teamIds,personId);
  if(refs.profile.code==="SYSTEM"&&await this.prisma.user.count({where:{profile:{code:"SYSTEM"},status:{code:"ACTIVO"}}})>0)throw new BadRequestException("Solo puede existir una cuenta SYSTEM activa");
 ...
 async update(id:string,i:Record<string,unknown>,administratorId:string){
  ...
  if(refs.profile.code==="SYSTEM"&&current.profile.code!=="SYSTEM"&&await this.prisma.user.count({where:{profile:{code:"SYSTEM"},status:{code:"ACTIVO"}}})>0)throw new BadRequestException("Solo puede existir una cuenta SYSTEM activa");
