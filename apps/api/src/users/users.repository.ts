...
export class UsersRepository{
 ...
 async options(){
  const [values,teams,people]=await Promise.all([
   this.prisma.catalogValue.findMany({where:{active:true,catalog:{code:{in:["PERFIL_USUARIO","ESTADO_USUARIO"]}}},orderBy:[{catalog:{code:"asc"}},{sortOrder:"asc"}],select:{id:true,code:true,name:true,catalog:{select:{code:true}}}}),
 ...
 async refs(profileId:string,statusId:string,teamIds:string[],personId:string|null,currentUserId?:string){
  const [profile,status,teams,person]=await Promise.all([
   this.prisma.catalogValue.findFirst({where:{id:profileId,catalog:{code:"PERFIL_USUARIO"}}}),
 ...
 async update(id:string,i:Record<string,unknown>,administratorId:string){
  ...
  if(current.profile.code==="ADMINISTRADOR"&&(refs.profile.code!=="ADMINISTRADOR"||refs.status.code!=="ACTIVO")){const activeAdmins=await this.prisma.user.count({where:{profile:{code:"ADMINISTRADOR"},status:{code:"ACTIVO"}}});if(activeAdmins<=1)throw new BadRequestException("No se puede desactivar o degradar al último administrador activo")}
