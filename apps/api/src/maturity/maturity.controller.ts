...
export class MaturityController {
  ...
  async createPeriod(@Body() body: { code: string; name: string; startDate: string; endDate: string; selfAssessmentOpensAt: string; selfAssessmentClosesAt: string; calibrationClosesAt: string; configurationVersion: string },@Headers("x-mochelab-demo-user-email") email?:string) {
    return this.repository.createPeriod(body,await this.access.getUserId("ADMINISTRADOR",email));
  }
  ...
  async transitionPeriod(@Param("id") id: string, @Body() body: { status: string },@Headers("x-mochelab-demo-user-email") email?:string) {
    return this.repository.transitionPeriod(id, body.status,await this.access.getUserId("ADMINISTRADOR",email));
  }
  ...
  async createDimension(@Body() body: { code: string; name: string; description?: string; weight: number },@Headers("x-mochelab-demo-user-email") email?:string) { return this.repository.createDimension(body,await this.access.getUserId("ADMINISTRADOR",email)); }

  @Patch("configuration/dimensions/:id")
  async updateDimension(@Param("id") id: string, @Body() body: { name?: string; description?: string; weight?: number; active?: boolean },@Headers("x-mochelab-demo-user-email") email?:string) { return this.repository.updateDimension(id, body,await this.access.getUserId("ADMINISTRADOR",email)); }

  @Post("configuration/behaviors")
  async createBehavior(@Body() body: { dimensionId: string; code: string; statement: string; helpText?: string; weight: number; roleIds: string[] },@Headers("x-mochelab-demo-user-email") email?:string) { return this.repository.createBehavior(body,await this.access.getUserId("ADMINISTRADOR",email)); }

  @Patch("configuration/behaviors/:id")
  async updateBehavior(@Param("id") id: string, @Body() body: { statement?: string; helpText?: string; weight?: number; active?: boolean; roleIds?: string[] },@Headers("x-mochelab-demo-user-email") email?:string) { return this.repository.updateBehavior(id, body,await this.access.getUserId("ADMINISTRADOR",email)); }
  ...
  calibrate(
    ...
    return this.access.getUserId("ADMINISTRADOR",email).then(administratorId=>this.repository.calibrate(parsed.data,administratorId));
  }
}
...
export class SelfAssessmentController {
  ...
  private scope(profile:string|undefined,email:string|undefined){return this.access?this.access.getTeamScope(profile?.toUpperCase()==="ADMINISTRADOR"?"ADMINISTRADOR":"USUARIO" as ProfileCode,email):Promise.resolve(null)}
  ...
  async form(@Query("personRoleId") currentPersonRoleId?: string,@Headers("x-mochelab-demo-profile") profile?:string,@Headers("x-mochelab-demo-user-email") email?:string) {
    ...
    const identityProfile:ProfileCode=profile?.toUpperCase()==="ADMINISTRADOR"?"ADMINISTRADOR":"USUARIO";
