...
export class MaturityRepository {
  ...
  async getCalibration(id: string) {
    const maturity = await this.prisma.roleMaturity.findUnique({ where: { id }, include: { level: true, period: {include:{status:true}}, person:true,role:true,selfAssessment: { include: { submittedBy:{select:{name:true,email:true}},modelVersion:{include:{assessmentModel:{select:{name:true}}}},responses: { orderBy: [{ dimensionCode: "asc" }, { behaviorId: "asc" }] }, resultDetails: { orderBy: [{ scopeType: "asc" }, { scopeCode: "asc" }] } } } } });
  ...
  async assistedAssessmentOptions(teamIds:string[]|null){
    const period=await this.prisma.period.findFirst({where:{active:true,status:{code:"AUTOEVALUACION"}},orderBy:{startDate:"desc"},include:{status:true}});
    if(!period)return{period:null,items:[]};
    const[assignments,submitted]=await Promise.all([this.prisma.personRole.findMany({where:{teamId:teamIds===null?undefined:{in:teamIds},status:{code:"ACTIVO"},developmentPathMode:"STANDARD"},include:{person:true,role:true,team:true},orderBy:[{person:{names:"asc"}},{role:{name:"asc"}},{team:{sourceId:"asc"}}]}),this.prisma.roleSelfAssessment.findMany({where:{periodId:period.id},select:{personId:true,roleId:true}})]);
    const submittedKeys=new Set(submitted.map(item=>`${item.personId}|${item.roleId}`)),groups=new Map<string,typeof assignments>();for(const item of assignments){const key=`${item.personId}|${item.roleId}`;groups.set(key,[...(groups.get(key)??[]),item])}
    return{period:{id:period.id,name:period.name,status:period.status.code},items:[...groups.entries()].map(([key,items])=>({personRoleId:items[0].id,personId:items[0].personId,person:items[0].person.names,dni:items[0].person.dni,roleId:items[0].roleId,role:items[0].role.name,teams:items.map(item=>item.team.sourceId),submitted:submittedKeys.has(key)}))};
  }

  async submitSelfAssessment(input: SubmitSelfAssessment,submission:{mode:"SELF"|"ADMIN_ASSISTED"|"FACILITATOR_ASSISTED";submittedById:string;reason?:string;method?:string;notes?:string;respondentConfirmed?:boolean}) {
    this.prisma.requireConnection();
    ...
    if(submission.mode!=="SELF"&&(!submission.reason?.trim()||!submission.method?.trim()))throw new BadRequestException("El motivo y el medio de asistencia son obligatorios");
    const submissionData={submissionMode:submission.mode,submittedById:submission.submittedById,assistanceReason:submission.mode==="SELF"?null:submission.reason!.trim(),assistanceMethod:submission.mode==="SELF"?null:submission.method!.trim(),assistedAt:submission.mode==="SELF"?null:new Date(),assistanceNotes:submission.mode==="SELF"?null:submission.notes?.trim()||null,respondentConfirmed:submission.mode==="SELF"?true:Boolean(submission.respondentConfirmed)};
