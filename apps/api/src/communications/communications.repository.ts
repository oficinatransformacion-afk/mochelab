...
export class CommunicationsRepository {
  ...
  async list(status?:string,page=1,pageSize=25){page=Math.max(1,page);pageSize=Math.min(100,Math.max(1,pageSize));const where={status:status||undefined};const[total,items]=await Promise.all([this.prisma.communication.count({where}),this.prisma.communication.findMany({where,skip:(page-1)*pageSize,take:pageSize,orderBy:{createdAt:"desc"},include:{personRole:{include:{person:true,role:true,team:true}},period:true,requestedBy:{select:{name:true,email:true}}}})]);return{items,total,page,pageSize,pages:Math.max(1,Math.ceil(total/pageSize))}}
